...
METADATA_DIR = '/home/shared/metadata'
TRAIN_DIR = '/home/shared/train'
...
def calibrate_magnetic_wifi_ibeacon_to_position(path_file_list):
    ...
    for path_filename in path_file_list:
        ...
        step_positions = compute_step_positions(acce_datas,
                                                ahrs_datas,
                                                posi_datas)
        # visualize_trajectory(posi_datas[:, 1:3], floor_plan_filename,
        #                      width_meter, height_meter, title='Ground Truth',
        #                      show=True)
        # visualize_trajectory(step_positions[:, 1:3], floor_plan_filename,
        #                      width_meter, height_meter, title='Step Position',
        #                      show=True)
...
def main(id_list, metadata_dir, train_dir):
    for this_id in id_list:
        floor_list = [
            name for name in os.listdir('{}/{}'.format(metadata_dir, this_id))
            if os.path.isdir('{}/{}/{}'.format(metadata_dir, this_id, name))
        ]

        for this_floor in floor_list:
            floor_data_dir = '{}/{}/{}'.format(metadata_dir, this_id, this_floor)
            path_data_dir = '{}/{}/{}'.format(train_dir, this_id, this_floor)
            floor_plan_filename = floor_data_dir + '/floor_image.png'
            floor_info_filename = floor_data_dir + '/floor_info.json'

            save_dir = './my_output/{}/{}'.format(this_id, this_floor)
            ...
            for path_filename in path_filenames:
                ...
                fig = visualize_trajectory(
                    path_data.waypoint[:, 1:3], floor_plan_filename, width_meter,
                    height_meter, title=path_id, show=False
                )
            ...
            fig = visualize_trajectory(
                step_positions, floor_plan_filename,width_meter, height_meter,
                mode='markers', title='Step Position', show=True
            )
            ...
            fig = visualize_heatmap(
                heat_positions, heat_values, floor_plan_filename, width_meter,
                height_meter, colorbar_title='mu tesla', title='Magnetic Strength',
                show=True
            )
            ...
            for target_ibeacon in ibeacon_rssi.keys():
                ...
                fig = visualize_heatmap(
                    heat_positions, heat_values, floor_plan_filename, width_meter,
                    height_meter, colorbar_title='dBm',
                    title=f'iBeacon: {target_ibeacon} RSSI', show=True
                )
            ...
            fig = visualize_heatmap(
                heat_positions, heat_values, floor_plan_filename, width_meter,
                height_meter, colorbar_title='number',
                title=f'Wifi Count', show=True
            )
...
if __name__ == "__main__":
    id_list = [name for name in os.listdir(METADATA_DIR)
               if os.path.isdir('{}/{}'.format(METADATA_DIR, name))]

    #queue = Queue()
    interval = math.ceil(len(id_list)/15)
    processes = [
        Process(target=main, args=(id_list[x:min(x+interval, len(id_list)-1)],
                                   METADATA_DIR,
                                   TRAIN_DIR))
        for x in range(0, len(id_list), interval)
    ]
